...
if (isset($_GET["sub"])) {
	...
	if ($_GET["sub"] == "urkundeAuto") {
		...
		if (isset($_SESSION["valueArray"])) {
			...
			if ($diszQuery && $row = mysqli_fetch_assoc($diszQuery)) {
		...
		if($_GET["sub"] == "urkundeManuell") {
			...
			for($i = 1; $i <= $numUrkunden; $i++) {
				...
				if($name != "") {
					$y = $y+$yPlus;
					$pdf->SetY($y);
					$pdf->Cell(0,0,utf8_decode($name),0,2,'C');
				}
				if($klasse != "") {
					$y = $y+$yPlus;
					$pdf->SetY($y);
	...
	echo "</table>";
		...
		if(isset($_POST["klassenstufe"])) {
			...
			// echo $query;
			// $query = "SELECT wert,schueler_id FROM werte WHERE schueler_id IN(" . $studentString . ") AND disziplin_id = '" . $diszID . "' ORDER BY CONVERT(wert,DECIMAL) " . $diszOrdering . " LIMIT " . $NumPeo;
